...
// PCL-DOWNSTREAM: non-destructive compaction — Compact is now a no-op,
// and GetFullHistory returns every message ever written to the session.
func TestCompact_PreservesAllMessages(t *testing.T) {
	...
	if full[0].Content != "a" || full[9].Content != "j" {
		t.Errorf("wrong full content: %+v", full)
	}
}
