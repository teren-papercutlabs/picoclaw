...
func newTestChannel(t *testing.T, exec testExecutor) *Channel {
	...
	cfg := config.HTTPChannelConfig{Enabled: true}
...
func TestStatusPending(t *testing.T) {
	// Use a blocking executor so the request stays pending
...
func TestWebhookPath(t *testing.T) {
	b := bus.NewMessageBus()
	defer b.Close()
	ch, _ := NewHTTPChannel(config.HTTPChannelConfig{Enabled: true}, b)
	path := ch.WebhookPath()
