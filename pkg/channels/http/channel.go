...
func NewHTTPChannel(cfg config.HTTPChannelConfig, b *bus.MessageBus) (*Channel, error) {
	base := channels.NewBaseChannel(channelName, cfg, b, nil)
	ch := &Channel{
		BaseChannel: *base,
		cfg:         cfg,
		bus:         b,
...
func (c *Channel) Send(_ context.Context, _ bus.OutboundMessage) error {
	// HTTP webhook channel is inbound-only; outbound messages are a no-op.
	return nil
}
